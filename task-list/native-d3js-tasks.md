...
- [x] Tailwind UI コンポーネントの作成
  - [x] @apply ディレクティブによるボタンコンポーネント
  - [x] @apply ディレクティブによるフォームコンポーネント
  - [x] @apply ディレクティブによるモーダルコンポーネント
...
- [ ] マインドマップデータモデルの定義
  ...
  - [x] ツリー構造の管理クラス（型安全な実装）
  - [x] ノードの一意 ID 生成機能（utils/helpers.ts）
  - [x] バリデーション機能（utils/validators.ts、型ガード実装）
  - [ ] インターフェース・型エイリアスの定義
- [x] 状態管理の主要機能
  - [x] ノード情報の管理（テキスト、色、位置、親子関係）
  - [x] 選択中ノードの管理
  - [x] 折りたたみ状態の管理
  - [x] キャンバス表示状態の管理（ズーム、位置）
  - [x] プロパティベーステスト導入（fast-check）
    - [x] UUID 生成・検証の包括的テスト
    - [x] キャンバス状態管理の堅牢性テスト
    - [x] 選択状態管理の一貫性テスト
    - [x] 折りたたみ状態管理の正確性テスト
